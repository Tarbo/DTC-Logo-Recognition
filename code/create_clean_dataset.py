...
if __name__ == '__main__':
    ...
    for subdir_counter, (r, subdirs, files) in enumerate(os.walk(xmlpath)):
        ...
        for filename in files:
            ...
            if (not os.path.isfile(imgpath)) or (im is None):
                ...
                print(
                    'Deleted xml for unavailable image file {:s}'.format(imgpath))
                continue
            imageCounter += 1
            ...
            os.rename(os.path.join(r, filename),
                      os.path.join(r, new_imagename+'.xml'))
            filename = new_imagename + '.xml'
            filewithpath = os.path.join(r, filename)
            os.rename(os.path.join(r, imagename + ext),
    ...
    print(f'>>> Processed folders: {subdir_counter:d}')
    #print('Processed rois: {:d}'.format(totalRoiCounter))
    print(f'>>> Processed images: {imageCounter:d}')
    print(f'>>> Processed brands: {len(set(brandlist)):d}')
    print(f'>>> Unavailable image files: {unavailableCounter:d}')
